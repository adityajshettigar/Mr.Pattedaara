...
| Action | Superintendent | IO | Analyst | Viewer |
|---|:---:|:---:|:---:|:---:|
| View all cases | ✅ | own cases | ✅ | ✅ |
...
### 1. Clone & Install
```bash
git clone [https://github.com/adityajshettigar/Mr.Pattedaara.git](https://github.com/adityajshettigar/Mr.Pattedaara.git)
cd Mr.Pattedaara
npm install
```
...
*Tip: Generate a secure JWT secret using `openssl rand -hex 64`*
...
All routes require `Authorization: Bearer <token>` header or `auth_token` cookie, except `/api/auth/login`.
...
Here is the completely resolved and merged `README.md`. I took the comprehensive, detailed documentation from your local version and seamlessly integrated the "What's New in v3" highlights from the remote branch. 

### 🌗 The GitHub Dark Mode Logo Fix
Because GitHub strips CSS filters (like `filter: invert(1)`), you cannot invert an image purely using Markdown code. The official GitHub standard to fix this is using the HTML `<picture>` tag to serve a different image based on the user's theme. 

I have updated the code at the very top of the README to use this method. **All you need to do is save an inverted/white version of your logo into your `public/` folder and name it `logo-white.png`.**

Copy and paste this entirely into your `README.md`:
